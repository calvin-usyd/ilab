...
var heatmap = (function heatmapClosure(){
	...
	function renderBar(){
		renderBarById('#heatmapTbl');
	}
	function renderBarById(htmlElemId){
		var colValArr = getColors(genBarVal());
		var colrTextArr = [], span, colrObj;
		var hmTblElem = $(htmlElemId);
		hmTblElem.empty();
		
		for(var i=_totalColor+1; i>0; i--){
			span = $('<span class="list-group-item small">');
			colrObj = colValArr[i-1];

			span.css({
				backgroundColor: colrObj.color,
				width: '100%'
			}).html(colrObj.value.toFixed(2)).appendTo(hmTblElem);
		}
	}
