...
this.removeAllNodeObjAndNo = function(){
	_.forEach(QF.setting.nodeObjArray, function(o){			
		o.o.removeChildren();
		stage.removeChild(o.o);
	});
...
this.removeAllElementObjAndNo = function(){
	...
	_.forEach(QF.setting.elementIndexArray, function(o){
		o.elem.removeChildren();
		stage.removeChild(o.elem);
	});	
	//RE-INIT ELEMENT INDEX
	QF.setting.elementIndexArray = [];
	QF.setting.elementNoTextArray = [];
}
this.minMaxNodeIndexInElem = function(){
	var nodeIndArr = [];
	_.forEach(QF.setting.elementIndexArray, function(elemO){
		_.forEach(elemO.nodes, function(indVal){
			nodeIndArr.push(indVal);
		});
	});
	return {
		max:_.max(nodeIndArr), 
		min:_.min(nodeIndArr)
	};
}
/*
nodeIndexArray = [
...
this.drawFEByRightClick = function(isClosed, prop){	
	...
	if (nodes.length >= 8){
		for (var x=4; x<8; x++){
		  nodeIndexArray.push(this.getNodeIndex({x:nodes[x].x, y:nodes[x].y}));
	    }
	}
...
this.drawElement = function(nodeIndexArray, isClosed, prop){
	nodeIndexArray = _.filter(nodeIndexArray, function(n) {
	...
	if (elemType == 1){
		console.log('WARNING: Only 1 nodes selected!');
		return -1;
	}
	...
	if(isClosed && elemType != 2){//CLOSED ELEMENT IF WAS TOLD TO CLOSE AND IF THERE ARE MORE THAN 2 NODES
		elem.lineTo(x1, y1);		
	}
	stage.addChild(elem);
...
this.drawNodeSnapGrid = function(mousePos){
	...
	mousePos = lg.snapToPoint(mousePos);
	var mapXY = dynamicP.user(mousePos.x, mousePos.y);
	mapXY = mapP.user(mapXY.x, mapXY.y);
	mousePos.u = {x:mapXY.x, y:mapXY.y};
...
this.drawNodeCommon = function(pos, isNewNodeObjArray){
	if (QF.setting.snapGrid){
		...
		if(nodeIndex == 0){//Do not draw node if already drawn
			...
			node.initVerticesNo();
			node.refreshConstraint(pos.c);
			node.userCoord = {x:pos.x, y:pos.y};
			stage.addChild(node);
			//if (isNewNodeObjArray) QF.setting.nodeObjArray.push({o:node, x:pos.x-grid.distX, y:pos.y+grid.distY});
			if (isNewNodeObjArray) QF.setting.nodeObjArray.push({o:node, x:pos.x, y:pos.y, u:pos.u, cC:pos.cC, pC:pos.pC});
...
this.getNodeIndex = function(node){
	...
	return _.findIndex(arr, {x:node.x.toFixed(2), y:node.y.toFixed(2)}) + 1;
}
