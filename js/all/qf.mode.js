...
return Object.freeze({
	...
	initEditor : function(){
		...
		lg.renderHotModal('#modalEditor', 'hotEditorNode');//Must trigger b4 popup modal coz its on 1st page
	},
	...
	settemperature : function(){
		$('#modaltemperature').modal().draggable({
		  handle: ".modal-header"
		});
	},
	setLoad : function(){
	...
	initPatrus : function(){
		...
		$('.solverType').html('PATRUS');
		cm.popUp('#modalSolver');		
	},
	initPafram : function(){
		QF.setting.solverType = 'PAFRAM';
		$('.solverType').html('PAFRAM');
		cm.popUp('#modalSolver');		
	},
	initPaisop : function(){
		QF.setting.solverType = 'PAISOP';
		$('.solverType').html('PAISOP');
		cm.popUp('#modalSolver');		
	},
	initSpas : function(){
	...
	initSimulation : function(){
		...
		}else if(sT == 'CONFEM' || sT == 'PATRUS' || sT == 'PAFRAM' || sT == 'PAISOP'){
			param = '';
	...
	initExe : function(){
		...
		if (pName == '' || typeof pName === 'undefined'){
			...
			cm.loadProgress();
			if ($('#useExternalServer').attr('data') === '1'){
				$.post(
					QF.setting.serv_exe[sT]['external'],
					{p:pName},
					function(json){
						if (json.length == 2){
							alert(json[1])
				...
				$.ajax({
					//url: QF.setting.serv_exe[sT]['local'] + pName + '&u=' + username + '&rand=' + (new Date()).getMilliseconds(),
					url: url + $('#solver_local_' + sT).attr('data') + pName + '&u=' + username + '&rand=' + (new Date()).getMilliseconds(),
					type: "POST",
					data: {p:pName},
				...
				//window.open(QF.setting.serv_exe[sT]['local'] + pName + '&u=' + username + '&r=' + (new Date()).getMilliseconds(), '_blank', 'menubar=0,toolbar=0,location=0,fullscreen=yes');
			}
			cm.hideProgress();
		}
	},
