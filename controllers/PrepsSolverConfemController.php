...
class PrepsSolverConfemController extends PrepsController{
	...
	function saveInputData(){
		...
		if ($this->useExternalServer){
			...
			$this->generateFile($this->projName . ".spa", $this->contentStr, $this->projName);
			$this->moveExe($this->projName);		
		}
	}
	
	//function solve($useExternalServer, $projName, $user, $ssh){
	function solve($projName){
		$projPath = $this->solverPath . '/'. $this->user . '/'. $projName . '/';
		if ($this->useExternalServer){
			echo $this->ssh->exec("./$this->solverPath/$this->solverName $projPath");
		}
	}
	
	function moveExe($projectName){
		$pNameSlug = Web::instance()->slug($projectName);
		$userPath = 'data/'.$this->user;
