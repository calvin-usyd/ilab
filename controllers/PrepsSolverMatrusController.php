...
class PrepsSolverMatrusController extends PrepsController{
	...
	function generateInputData($postData, $propArray, $consArray, $user){
		...
		$this->generateFile("particles.txt", $vertStr, $projectName);
		$this->generateFile("tags.txt", $tagsStr, $projectName);
		$this->generateFile("parameters.txt", $this->generateDataStringWithLabel($parameterData, $this->newLine), $projectName);
		$this->generateFile("project_parameters.txt", $this->generateDataStringWithLabel($solverVal['sim'], $this->newLine), $projectName);
